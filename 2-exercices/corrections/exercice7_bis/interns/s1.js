...
function noteEleve () {
    ...
    for (let i = 0; i < 5 ; i++) {
        let notedelEleve;
        do {
            notedelEleve = parseInt(prompt(`Saisissez la note de ${matières[i]} de l'élève`));
...
while (note != -1) {
    note = parseInt(prompt("Souhaitez vous saisir les notes d'un elève ?(Saisir n'importe quelle touche pour oui, -1 pour arrêter la saisie)"));
    if (note != -1) {
        eleves.push(noteEleve());
    } else {
        console.log(`La moyenne de chaque élève est de ${moycommente(moyenneEleve(eleves))}`);
        console.log(`La moyenne de chaque matière est de ${moyenneMatiere (eleves)}`);
    }
}